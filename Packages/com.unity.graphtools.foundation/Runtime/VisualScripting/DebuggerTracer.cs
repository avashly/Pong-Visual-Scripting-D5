...
#pragma warning restore 414
    ...
    public class FrameData
    {
        ...
        public IEnumerable<EntityReference> Entities => EntityFrameData.Keys;

        public EntityFrameTrace GetEntityFrameTrace(int eIndex, bool createIfAbsent)
        {
            ...
            if (!EntityFrameData.TryGetValue(eRef, out var data) && createIfAbsent)
                EntityFrameData.Add(eRef, data = new EntityFrameTrace());
            return data;
        }
    }
    ...
    public static IEnumerable<FrameData.EntityReference> GetTargets(int frameCount, int vsGraphAssetModelId)
    {
        var graphData = GetGraphData(vsGraphAssetModelId, false);
        return graphData?.GetFrameData(frameCount, false)?.Entities ?? Enumerable.Empty<FrameData.EntityReference>();
    }
